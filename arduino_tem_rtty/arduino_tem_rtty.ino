//45.45baud BAUDOT TELEMETRY BEACON 1.4
...
const char ID[] = "DE CA1CVI CA1CVI CA1CVI /B ";
...
char    ch;
char    sVal[9];
char    Str4[ ] ="hola"; // agregado por mi
float hola = analogRead(0);
...
void loop()
{
        ...
        if(sSq == 0)
        {
                if( sRd == 1)
                {
                        if(i == 0)
                        {
                                sprintf(sVal, "A0: %04d", hola); //aca sd modifico la lectura por una variable
                        }
                        if(i == 1)
                        {
                ...
                if(ch == '\0')
                {
                        ...
                        sendFsk();
                        i++;
                        if(i == 4) //OVERFLOW depende de la cantidad de sensores a transmitir de 0 a 3 4 salta
                        {
                                i = 0;
                                sSq = 1;
                        }
        ...
        if(sSq == 1)
        {
                if(idC < 3)   //este es el contador de veces a trasnmitir los datos era 9
                {
                ...
                if(idC == 3) //valor del contador a modificar para variar el contador, 
                {            //se debe modificar arriba igual era 9
                        ch = ID[j];
                        chTable();
                        if(ch != '\0')
                        {
