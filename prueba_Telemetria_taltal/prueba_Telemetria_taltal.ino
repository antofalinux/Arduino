...
//
// Adaptado y ampliado por EB1HBK y EB1WM, entre Mayo y Junio de 2011.
//
// << -- Keyed CW mode -- >>
//**************************************************
...
// si el dpin 11 esta en HIGH (hay tension de red) en la telemetria no se envia ya que esta interlockeado.
...
vbat = (adc2*0.02); // Obtiene el valor en voltios de la bateria en el pin 2 despues del divisor 1/4
  ...
  if(hot == 1){
      ...
      Serial.println("Calentando Cuarzo 3 Seg."); 
      delay(3000);  // Espera 30 segundos para que la PTC caldee el cuarzo. NOTA: en vez de tiempo podria
  ...
  red220();      // ejecuta la transmision de la baliza
    ...
    Serial.println("DURMIENDO POR 10 MIN");
...
void baliza()
{
  ...
  Serial.println("Se Activa PTT");
    ...
    for (cw=0;cw<adclen;cw++)sendcw(asciibuf[cw]); 
  ...
  delay(1000);  // Hace una pausa de 1 segundos
 
  // Activa el PTT y transmite un tono CW contÃ­nuo de 5 segundos
  ...
  delay(600000); // Wait 10 minutos before sending telemetry
  Serial.println("YA PASARON 10 MINUTOS");
  delay(600000); // Wait 10 minutos before sending telemetry
  Serial.println("YA PASARON 20 MINUTOS");
  delay(600000); // Wait 10 minutos before sending telemetry
  Serial.println("YA PASARON 30 MINUTOS");
...
for(j=0;j<3;j++);  //cuenta de 1 a 4 si es menor a 4 espera 5 min. si ya comprobo eso deja la variable en cero
 Serial.println (j); // imprime el valor de j 
     if (j <4){
 digitalWrite(ptt, HIGH);  //Activa la salida PTT y espera 125 msg antes de enviar CW
  tone (tonepin,tone2);
  delay(500);   
 digitalWrite(cwoutPin, HIGH); 
  delay(5000);
  digitalWrite(ptt, LOW);  //Activa la salida PTT y espera 125 msg antes de enviar CW
  tone (tonepin, LOW);  
 digitalWrite(cwoutPin, LOW); 
      delay (3000);
     }
      else  //si es 4 borra la variable y envia a baliza.
      j=0;
  baliza();
  }
...
void inicio()
{
  ...
  digitalWrite(ptt, HIGH);  //Activa la salida PTT y espera 250 msg antes de enviar CW
  tone (tonepin,tone2);
  delay(250);
  Serial.println("Se Activa PTT");
    { 
