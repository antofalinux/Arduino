  ...
  if(Serial.available())
  {
    ...
    else if(incoming == '@') //Special character from Imp indicating midnight local time
    {
      ...
      //Serial.print("Midnight reset");
        ...
        else if(incoming == '%') //obtencion de la presion
    {
      pressWeather(); //Reset a bunch of variables la presion absoluta
      //Serial.print("valor de la direccion del viento");
    }
   else if(incoming == '$') //Special character for indicating velocidad viento
    {
      windspeedWeather(); //Reset a bunch of variables like rain and total rain
      //Serial.print("valor de la velocidad del viento");
   } 
...
//enviando datos de direccion del viento
void pressWeather()
{
...
// float Tr;
  float humedad;
 // humedad =humidity/100;
  //Tr = tempf - ((100 - humidity)/5);
//Tr = temperature - ((100 - humidity)/5);
  //Calc light level
...
void windspeedWeather()
{
  //int winddir; 
 float windSpeed; //= myPressure.readPressure();
// Serial.print("Pressure(Pa):");
...
void reportWeather()
{
 ...
 float pressure = myPressure.readPressure();
 pressure *=0.000295301;
 float Tr;
 Tr = tempf - ((100 - humidity)/5);
// Serial.print("Pressure(Pa):");
  Serial.print(pressure, 2); //Don't print pressure= because the agent will be doing calcs on the number
  //Serial.print(pressure, 2);
