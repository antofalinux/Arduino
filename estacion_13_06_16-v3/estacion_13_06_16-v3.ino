/* 
 Weather Station por Antofalinux
 By: Carlos Villalobos
 SingCall: CA1CVI
 Date: October 2th, 2015
...
//esta wea la copie del original no se que hace
//hasta aca
...
void setup()
{
  ...
  Serial.begin(9600);
   imp.begin(9600); // Debug
   imp.setTimeout(5000); 

    Serial.println("");
  Serial.println("");
  Serial.println("Inicializando Sistema ");
  Serial.print("Conectando a ESP");  
  Serial.println("");
  Serial.println("");
...
String datos1;
  ...
  datos1+=(winddir);
  datos1+=("&windspeedmph=");
  datos1+=(windspeedmph, 1);
  datos1+=("&windgustmph=");
  datos1+=(windgustmph, 1);
  datos1+=("&windgustdir=");
  datos1+=(windgustdir);
  datos1+=("&windspdmph_avg2m=");
  datos1+=(windspdmph_avg2m, 1);
//  imp.println("#");
//  delay(100);
  datos1+=("&winddir_avg2m=");
  datos1+=(winddir_avg2m);
  datos1+=("&windgustmph_10m=");
  datos1+=(windgustmph_10m, 1);
  datos1+=("&windgustdir_10m=");
  datos1+=(windgustdir_10m);
  datos1+=("#");
  imp.println(datos1);
  Serial.print(datos1);
...
//  Serial.println(pressure, 2);
  windgustmph = 0; //Zero out the windgust for the day
  windgustdir = 0; //Zero out the gust direction for the day
  rainin=0;        //Zero out the rainin for the day

  
  //Test string
  //Serial.println("$,winddir=270,windspeedmph=0.0,windgustmph=0.0,windgustdir=0,windspdmph_avg2m=0.0,winddir_avg2m=12,windgustmph_10m=0.0,windgustdir_10m=0,humidity=998.0,tempf=-1766.2,dewpoint=0,00,rainin=0.00,dailyrainin=0.00,-999.00,batt_lvl=16.11,light_lvl=3.32,#,");
}
...
double portId ;
  ...
  if (imp.available())
  {
    ...
    byte size = imp.readBytes(input, INPUT_SIZE);
